...
class StudentAccount {
	private double totalBalance; //stores student's outstanding balance for their tuition
	private String courses = ""; //store's student's desired course to enroll in
	private int costOfCourse = 200; //cost of each course offered at the university 
	...
	public void enroll() {
		...
		console.close(); //closes scanner and prevents resource leaks
	}
	...
	void viewAndPayBalance() {
		...
		while (!done) {
			System.out.println("You have an outstanding balance of $" + String.format("%.2f", totalBalance)); //shows current outstanding balance 
			//checks if student even has an outstanding balance
			if (totalBalance > 0) {
				System.out.println("Would you like to make payment? (y/n) "); //asks student if they would like to make a payment at this time
				answer = input.next();
				//if student answers yes, gather payment from student and update total account balance
				if (answer.equals("y") || answer.equals("Y")) {
					...
					if (amountPaid > totalBalance) //if student pays more than they owe, then displays error message and student is taken to beginning of loop
						System.out.println(
								"Error: Cannot pay an amount greater than the total balance. Please try again.\n");
					else {
						totalBalance = totalBalance - amountPaid; //total balance is updated after student chooses valid payment amount
						System.out.println("Thank you for submitting your payment!");
					}

				} else if (answer.equals("n") || answer.equals("N")) // if student answers no, exits loop and ends function ends
					done = true;
				else //if student inputs anything other than 'y' or 'n', error message displayed and loop restarts
